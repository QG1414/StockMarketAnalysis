...
class ChangeGraphs:
    ...
    def get_price_chart(current_company: pd.DataFrame, fullName: str):
        ...
        buyline, sellline = ChangeGraphs.linesBuySell(current_company["Close"])

        figure.add_trace(
            go.Scatter(
                x=current_company["Date"],
                y=buyline,
                mode="lines",
                line=dict(shape="spline", smoothing=0.1, color="red"),
                name="Buy Signal",
            )
        )
        figure.add_trace(
            go.Scatter(
                ...
                y=sellline,
                mode="lines",
                line=dict(shape="spline", smoothing=0.1, color="green"),
                name="Sell Signal",
            )
        )
        ...
        figure.update_layout(
            ...
            title=f"Stock Market Prices {fullName}",
            xaxis_title="Date",
    ...
    def get_avarages_chart(current_company: pd.DataFrame, fullName: str):
        figure = px.line(
            ...
            y=["Close", "MA10", "MA20"],
            template="plotly_dark",
            title=f"Moving Avarages{fullName}",
        )
    ...
    @staticmethod
    def linesBuySell(source: pd.DataFrame):
        buyline = []
        sellline = []
        isBuy = False
        isSell = False
        ...
        for i in range(start, -1, -1):
            if not isBuy:
                isBuy = (source.iloc[i] > source.iloc[i + 1]) and (
                    source.iloc[i + 1] < source.iloc[i + 2]
                )
                if isBuy:
                    isSell = False

            if not isSell:
                isSell = (source.iloc[i] < source.iloc[i + 1]) and (
                    source.iloc[i + 1] > source.iloc[i + 2]
                )
                if isSell:
                    isBuy = False
